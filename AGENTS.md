...
| 平台 | 命令 |
|------|------|
| **macOS/Linux** | `ln -s $(pwd)/skill-installer ~/.kimi/skills/skill-installer` |
| **Windows** | `New-Item -ItemType SymbolicLink -Path "$env:USERPROFILE\.kimi\skills\skill-installer" -Target "$(pwd)\skill-installer"` |
...
如果你还没有 skill-installer，需要**手动创建软连接**（见上方步骤 2）。
...
### TODO-15：修复 Windows 跨驱动器路径计算错误

**问题描述**：
运行 `python skill-installer/scripts/generate_plan.py --skill skill-installer --action install --option full` 时出现错误：
```
...
**问题原因**：
- `platform_utils.py` 的 `calculate_relative_path()` 使用 `os.path.relpath()` 计算相对路径
- 当源路径（项目在 `D:` 盘）和目标路径（Kimi skills 在 `C:` 盘）位于不同驱动器时
- `os.path.relpath()` 在 Windows 上抛出 `ValueError`

**修复方案**（方案 B - 显式平台检测）：

修改 `skill-installer/scripts/lib/platform_utils.py` 第 103-122 行：
1. 添加 Windows 平台显式检测 `PlatformInfo.is_windows()`
2. 比较驱动器盘符 `from_abs.drive != to_abs.drive`
3. 跨驱动器时返回绝对路径（软连接支持绝对路径）
4. 同驱动器或非 Windows 时正常计算相对路径

**跨平台兼容性**：
| 平台 | 场景 | 预期行为 |
|------|------|----------|
| Windows | D: → C:（跨驱动器） | 返回 D: 盘绝对路径 |
| Windows | C: → C:（同驱动器） | 返回相对路径 |
| macOS/Linux | 任意路径 | 返回相对路径（无盘符概念）|

**任务清单**：

| 序号 | 任务 | 状态 | 说明 |
|------|------|------|------|
| 1 | 修改 `platform_utils.py` | ✅ 已完成 | 重写 `calculate_relative_path` 方法 |
| 2 | 运行 L1 代码测试 | ✅ 已完成 | `test_platform_utils.py` 全部通过 |
| 3 | 功能验证 | [待用户] | 请运行 `generate_plan.py` 验证修复效果 |
...
以下原则适用于所有开发工作，确保高质量交付：

---

## Session 启动强制检查

每次会话开始时的强制性流程，确保原则不被忽视：

1. **强制阅读 AGENTS.md**
   - 每个 session 开始时，必须完整阅读项目 AGENTS.md
   - 不得跳过任何章节
   - 特别关注其中的"工作原则"和"约束"部分

2. **提取关键约束到上下文**
   - 明确复述 AGENTS.md 中的关键工作原则
   - 确认理解各项约束的含义

3. **自检清单确认**
   - 在 AGENTS.md 中查找"Session 启动检查清单"
   - 按清单逐项确认，未完成禁止执行操作

4. **主动承诺**
   - 明确声明已阅读并理解 AGENTS.md
   - 承诺遵循其中的工作原则
...
1. **检查原则冲突**
   - 此工具的使用方式是否与 AGENTS.md 中的原则冲突？
   - 例如：
     - 使用 SetTodoList 工具 vs AGENTS.md TODO 管理原则
     - 直接执行 vs 方案先行原则
     - 批量修改文件 vs 最小变更原则

2. **冲突处理**
   - **如有冲突**：优先遵循 AGENTS.md，禁止使用该工具
   - **如无冲突**：正常执行
   - **不确定时**：立即停止，询问用户

3. **记录工具使用理由**
   - 对于每个工具调用，确保有明确的使用理由
   - 该理由不与 AGENTS.md 中的任何原则冲突
...
| 原则 | 常见违规场景 | 检查项 |
|------|-------------|--------|
| 方案先行 | 用户说"开始吧"后直接执行 | ☐ 是否已展示方案并获确认？ |
| TODO 管理 | 使用 SetTodoList 而非 AGENTS.md | ☐ 是否已将 TODO 写入 AGENTS.md？ |
| 文件数量限制 | 一次修改超过 3 个文件 | ☐ 是否需要拆分子任务？ |
| 最小变更 | 修改超出需求范围的文件 | ☐ 每个修改是否都有明确理由？ |
| 测试规范 | 测试前未将测试计划加入 TODO | ☐ 测试前是否已将测试计划加入 AGENTS.md 的 TODO？ |
| 测试规范 | 测试后未更新 TODO 状态 | ☐ 测试完成后是否已更新 AGENTS.md 中的 TODO 状态？ |
| 测试规范 | 修改后未验证 | ☐ 是否已验证修改结果？ |
| 工作目录限制 | 操作超出项目目录 | ☐ 路径检查是否通过？ |
...
> macOS/Linux 开发者请忽略本节，参考"跨平台兼容原则"

Windows 平台下的命令执行要求：

1. **使用 PowerShell 风格**
   - 优先使用 PowerShell 语法和命令风格
   - 避免使用 bash 特有的语法（如 `&&`, `||`, `;` 链式命令）

2. **路径处理**
   - 使用反斜杠 `\` 或 PowerShell 的路径拼接
   - 避免硬编码 Unix 风格路径分隔符

3. **命令分隔**
   - 使用分号 `;` 分隔多个 PowerShell 命令
   - 或使用 PowerShell 的管道 `|` 进行链式操作
